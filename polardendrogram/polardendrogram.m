...
% set(h, labels(varargin{2}),'FontSize',6);
...
%     text(x,y,num2str(perm(i)));
    ...
    [x,y]=pol2cart((((i-minx)/xrange)*(pi*11/6))+(pi*1/13),1.01);
    THETA = (((i-minx)/xrange)*330 + 18); % pi/12 = 15°
    cl = [.5 .5 .5]; fw = 'bold';sz =12;
    switch perm(i)
        case num2cell(1:25)
            cl = [.8 .06 .8];fw = 'bold';sz=14;
        case num2cell(26:50)
            cl = [.06 .4 .8];fw = 'bold';sz=14;
        case num2cell(51:75)
            cl = [.8 .4 .06];fw = 'bold';sz=14;
        case num2cell(76:100)
            cl = [.06 .8 .4];fw = 'bold';sz=14;
    end
    text(x,y,labels(perm(i)), 'rotation', THETA, 'Color',cl,'FontWeight',fw,'FontSize',sz);
%     text(x,y,num2str(perm(i)), 'rotation', THETA);
